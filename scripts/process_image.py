...
def main():
    ...
    if 'txt' in output_formats:
         if input_image_array is not None: output_handler.organize_output_by_id(results_dir, processed_data['id'], args.input, input_image_array, processed_data['extracted_data'])
         else: logger.error(f"Could not load image {args.input} to save standard TXT output.")
    if 'csv' in output_formats or 'db' in output_formats:
        aggregated_dict = {}
        if processed_data['status'] != "Failure" and processed_data['id'] not in ["PROCESSING_FAILED", "NO_DETECTIONS", "ID Extraction Failed", "UNKNOWN_ID", "OCR_EMPTY", "CROP_FAILED"]:
             aggregated_dict[processed_data['id']] = processed_data['extracted_data']
        if 'csv' in output_formats:
             if aggregated_dict: csv_path = os.path.join(results_dir, config.get('csv_filename', 'usage_report.csv')); output_handler.save_to_csv(aggregated_dict, csv_path)
             else: logger.warning("No valid data to save to CSV for this image.")
        if 'db' in output_formats:
             valid_results_for_db = [processed_data] if processed_data['status'] != "Failure" and processed_data['id'] not in ["PROCESSING_FAILED", "NO_DETECTIONS", "ID Extraction Failed", "UNKNOWN_ID", "OCR_EMPTY", "CROP_FAILED"] else []
             ...
             else: logger.warning("Skipping DB save due to processing failure or invalid ID.")
    ...
    if debug_mode:
        ...
        if processed_data['status'] != 'Failure':
            if input_image_array is None: input_image_array = image_utils.load_image(args.input)
            if input_image_array is not None:
                ...
                if success: logger.info(f"Debug visualization saved to: {debug_image_path}")
            else: logger.error("Could not load input image for debug visualization.")
